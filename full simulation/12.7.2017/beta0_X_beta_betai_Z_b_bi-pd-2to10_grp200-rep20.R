...
run_one_sample <- function(iter){
  ...
  nRep.sim <- 20 # 50 # Duplication in each group n_i 
  epsilon.sd <- 1 # or 0.5
  intercept.true <- 0.5
  # nRandCovariate <- 
  b.sim <- 2
...
for(nRandCovariate in 1:5 * 2){  # START out-outer loop
  ...
  fileName <- paste("beta0_X_beta_betai_Z_b_bi-pd-2to10_grp200-rep20-", nRandCovariate,".RData", sep = "") # Saving file's name
