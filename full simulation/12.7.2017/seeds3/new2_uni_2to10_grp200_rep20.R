...
run_one_sample <- function(iter){
  ...
  for(k in 1:nGroup){
    svd <- svd(z.sim[((k-1)*nRep.sim+1):(k*nRep.sim), ] %*% t(z.sim[((k-1)*nRep.sim+1):(k*nRep.sim), ]))
    u.tra <- t(svd$v)
    u <- svd$u
...
for(nRandCovariate in 1:5 * 2){  # START out-outer loop
  clusterExport(cluster, c("nRandCovariate")) # casting the coefficient parameter on the random effects' covariance function
  fileName <- paste("new2_uni_beta0_X_beta_betai_Z_b_bi-pd-2to10_grp200-rep20-", nRandCovariate,".RData", sep = "") # Saving file's name
