...
fileName <- "beta0_X_beta_Z_b_ID.RData" # Saving file's name
...
run_one_sample <- function(iter){
  ...
  nRep.sim <- 100 # Duplication in each group n_i 
  epsilon.sd <- 1 # or 0.5
  intercept.true <- 0.5
  nRandCovariate <- 1
  b.sim <- 2
  ...
  dummyX <- rbinom(n = totalN, size = 1, prob = 0.5)
  Y.sim <- intercept.true + z.sim * b.sim + dummyX * beta.sim + error.sim # NEW add 'dummyX'
...
#  tests1 <- exactRLRT(noDummyXReml.pdIdent, fullReml.pdIdent, noZReml.pdIdent) # , nsim = 100000
  # Test intercept random
  tests1 <- exactRLRT(noZReml.pdIdent) # , nsim = 100000
  
  
  
  # 'lmer' model
  fullReml <- lmer(Y ~ 1 + dummyX + z.sim + (0 + dummyX | ID) + (0 + z.sim | ID), data = designMatrix)
  m0 <- update(fullReml,  . ~ . - (0 + z.sim | ID))
#  m.slope <- update(fullReml, . ~ . -  (0 + dummyX | ID))
  m.slope <- lmer(Y ~ 1 + dummyX + z.sim + (1 | ID), data = designMatrix)
  tests2 <- list()
#  tests2[[1]] <- exactRLRT(m.slope, fullReml, m0) # , nsim = 100000
  # Test intercept random
  tests2[[1]] <- exactRLRT(m.slope) # , nsim = 100000
