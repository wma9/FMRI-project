...
sample.sim <- 1000
pvalue.total <- .05
fileName <- "simu_var0_100_20_1000.RData"
rRange.sim <- c(0)
...
run_one_sample <- function(numSim){
  ...
  nTime.sim <- 80 # J
  nSubj.sim <- 100 # 200 # I the number of curves
  ...
  if(pca.npc == 1){
    ...
    notempReml.pdIdent <- lme(fixed = rating ~ 1 + temp + a.score,
                              ...
                              data = designMatrix.pdIdent,
                              control = lmeControl(msVerbose = TRUE, opt = 'optim', singular.ok=TRUE, returnObject=TRUE))
    # noAScoreReml.pdIdent <- update(fullReml.pdIdent, . ~ . - (0 + a.score|ID))
    # notempReml.pdIdent <- update(fullReml.pdIdent, . ~ . - (0 + temp|ID))
  }else if(pca.npc == 2){
...
#  tests1 <- exactRLRT(notempReml.pdIdent) 
  ...
  for(i in 1:pca.npc){
    ...
    f.slope <- as.formula(paste("rating ~ 1 + temp + a.score.", additive0.sim, " + (0 +", ii, " | ID)", 
                                sep = ""))
#    m.slope <- lmer(rating ~ 1 + temp + I(a.score) + (0 + I(a.score) | ID), data = designMatrix.lmm)
    m.slope <- update(fullReml, . ~ . -  (0 + temp | ID))
    tests2[[i]] <- exactRLRT(m.slope, fullReml, m0)
#    tests2[[i]] <- exactRLRT(m.slope)
  }
