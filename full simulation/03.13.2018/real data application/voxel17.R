...
npc <- results$npc # voxel=17 then npc=9
...
mseNoRandom <- mean((fmridata$rating - predict(NoRandomReml))^2) #6727.099  
# npc=9
Random.sim1 <- as.formula(paste("rating ~ 1 + temp.1 + temp.2 + ascore.", 
                                noRandom.simpart, 
                                " + (0 + temp.1 | ID) + (0 + temp.2 | ID)", 
                                paste0("+(0 + ascore.",1:npc," | ID)",collapse=""),
                                sep = ""))
RandomReml <- lmer(Random.sim1, data = designMatrix1)
mseRandom <- mean((fmridata$rating - predict(RandomReml))^2) #6654.186 
...
msefullReml<- mean(residuals(fullReml)^2) #6723.586
...
par(mgp=c(2,1,0), mar=c(5, 3, 3, 1) + 0.1, mfrow=c(1,1))
...
legend(x = "bottomleft",inset = 0,
       legend = c(paste("Subject", 1:nSubj, sep = ""), "Mean function"), 
...
######  violin plot ######
#resi2_fullReml <- residuals(fullReml)^2
...
ggplot(violin1, aes(x=Model, y=MSE)) +
  ...
  scale_x_discrete(breaks=c("1", "2", "3"),
                   labels=c("No ranef","Equal ranef", "Different ranef")) +
  labs(title="Voxel 19") + 
  theme_classic() +
...
plot(t, beta2[,2],type="l", ylim=c(f_ymin,f_ymax),
     ...
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 1 (Voxel 19)", col="blue", lwd=2)
...
plot(t, beta2[,5],type="l", ylim=c(f_ymin,f_ymax),
     xaxt='n', ylab=expression(beta(t)+beta[i](t)), xlab="Time (Seconds)", lty=1, 
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 4 (Voxel 19)", col="blue", lwd=2)
...
plot(t, beta2[,10],type="l", ylim=c(f_ymin,f_ymax),
     xaxt='n', ylab=expression(beta(t)+beta[i](t)), xlab="Time (Seconds)", lty=1, 
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 9 (Voxel 19)", col="blue", lwd=2)
...
plot(t, beta2[,20],type="l", ylim=c(f_ymin,f_ymax),
     xaxt='n', ylab=expression(beta(t)+beta[i](t)), xlab="Time (Seconds)", lty=1, 
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 19 (Voxel 19)", col="blue", lwd=2)
