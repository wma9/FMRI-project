...
mseNoRandom <- mean((fmridata$rating - predict(NoRandomReml))^2) #6504.301   7625.812
# npc=9
...
mseRandom <- mean((fmridata$rating - predict(RandomReml))^2) #5572.025 6312.707
...
RMSE_RandomReml <- calculateRMSE_i(fmridata$rating, predict(RandomReml)) #74.63737      79.42739
RMSE_NoRandomReml <- calculateRMSE_i(fmridata$rating, predict(NoRandomReml)) #80.60125  87.25439

library(MuMIn)
r.squaredGLMM(RandomReml) #0.65     0.6043441
r.squaredGLMM(NoRandomReml) #0.60   0.5289576
