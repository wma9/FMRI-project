...
npc <- results$npc # voxel=1 then npc=8
...
mseNoRandom <- mean((fmridata$rating - predict(NoRandomReml))^2) #6278.428  
# npc=8
...
RMSE_RandomReml <- calculateRMSE_i(fmridata$rating, predict(RandomReml)) #73.65064
RMSE_NoRandomReml <- calculateRMSE_i(fmridata$rating, predict(NoRandomReml)) #79.18292
library(MuMIn)
r.squaredGLMM(RandomReml) #0.6656895
r.squaredGLMM(NoRandomReml) #0.6086918
...
msefullReml<- mean(residuals(fullReml)^2) #5502.77
...
mean((yhat_fullRemlBack-fmridata$rating)^2) #5502.77
...
betai <- efunctions %*% theta_ik  #23*8 x 8*20 = 23*20
...
resi2_NoRandomReml <- residuals(NoRandomReml)^2
 violin_RandomReml <- violin_NoRandomReml <- c()
for (i in 1:length(nRep)){
  if(i==1){
...
violin1 <- data.frame(cbind(c(violin_NoRandomReml, violin_RandomReml),
                            rep(1:2, each=nSubj)))
...
violin1_resi <- data.frame(cbind(c(resi_NoRandomReml, resi_RandomReml),
                                 rep(1:2, each=dim(fmridata)[1])))
...
ggplot(violin1_resi, aes(x=Model, y=Residual)) +
  ...
  scale_x_discrete(breaks=c("1", "2"),
                   labels=c(expression(beta[i]*"(t)=0"), "proposed")) +
  labs(title="ROI 1") + 
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, size = rel(0.9), face="bold")) +
  theme(axis.title.y = element_text(size = rel(0.8))) +
...
plot(t, beta2[,2],type="l", ylim=c(f_ymin,f_ymax),
     ...
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 1 (ROI 1)", col="blue", lwd=2)
...
plot(t, beta2[,5],type="l", ylim=c(f_ymin,f_ymax),
     xaxt='n', ylab=expression(beta(t)+beta[i](t)), xlab="Time (Seconds)", lty=1, 
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 4 (ROI 1)", col="blue", lwd=2)
...
plot(t, beta2[,10],type="l", ylim=c(f_ymin,f_ymax),
     xaxt='n', ylab=expression(beta(t)+beta[i](t)), xlab="Time (Seconds)", lty=1, 
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 9 (ROI 1)", col="blue", lwd=2)
...
plot(t, beta2[,20],type="l", ylim=c(f_ymin,f_ymax),
     xaxt='n', ylab=expression(beta(t)+beta[i](t)), xlab="Time (Seconds)", lty=1, 
     cex.lab = 0.8, cex.axis=0.8,cex.main=0.8, cex.sub=0.8,
     main="Suject 19 (ROI 1)", col="blue", lwd=2)
