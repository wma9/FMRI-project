...
# npc=9
heter_additive <- paste0("+ (0 + ascore.",1:npc,"|ID)", collapse="")
...
calculateRMSE_i <- function(Y, EST){
  ...
  for(i in unique(designMatrix1$ID)){
    mse[i] <- mean(out[which(designMatrix1$ID==i)])
  }
  return(sqrt(mean(mse)))
}

RMSE_RandomReml <- calculateRMSE_i(fmridata$rating, predict(RandomReml)) #54.19158
RMSE_NoRandomReml <- calculateRMSE_i(fmridata$rating, predict(NoRandomReml)) #56.80106
library(MuMIn)
r.squaredGLMM(RandomReml) #0.6316781
r.squaredGLMM(NoRandomReml) #0.6002371
