...
b.var <- c(0.23^2) # The set of varaince of random covariates b as random slope
...
run_one_sample <- function(iter){
  ...
  if(smooth == 0){
    Merror.Var <- sum(lambdaVec.true) / SNR  #SNR = sum(lambdaVec.true)/Merror.Var,Merror.Var=0.583333 
...
fileName <- paste("f_power_", smooth, "_",b.var,"_grp20-rep20.RData", sep = "") # Saving file's name
...
npcdist <- table(sapply(node_results,function(x) x$npc))
power2.sim[[loopIndex]] <- list(Power = Power2, realTau = r.sim, smooth = smooth, ncp = npcdist)
