...
run_one_sample <- function(iter){
  ...
  Mt.true <-  ascore.true %*% psi.true + matrix(rnorm(totalN*D, mean=0, sd = sd.epsilon), totalN, D)       
  ...
  amply <- sum(lambdaVec.true) / (sd.epsilon^2 * SNR * 2)
  
  Y <- delta.true + dummyX * gammaI.true + (dummyX - 1) * gammaI2.true + betaM.true + error*amply 
  
  ##########################################################################
  
  ID <- rep(1:nSubj, each = nRep)
  M <- Mt.true
  M <- M - matrix(rep(colMeans(M), each = totalN), totalN, D, byrow = TRUE) # center:column-means are 0
...
fileName <- "f_seed2_grp20-rep20.RData" # Saving file's name
