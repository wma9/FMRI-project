...
sample.sim <- 500
pvalue.total <- .05
fileName <- "simu_var0_50_50_SNR3_0824.RData"
rRange.sim <- c(0)
...
run_one_sample <- function(iter){
  ...
  set.seed(iter)
  nTime.sim <- 80 # grid number
  nSubj.sim <- 50 # 200 # I the number of curves
  nRep.sim <-  50 # 20 # datasets for each covariance function 
  sd.epsilon <- 1 # or 0.5
  true.delta <- 0.5
  true.npc <- 3
  percent <- 0.95
  SNR.sim <- 3 # 5, signal noise ratio
  beta.sim <- 2
  a.mean <- 1
