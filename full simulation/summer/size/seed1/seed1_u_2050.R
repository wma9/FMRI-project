...
run_one_sample <- function(iter){
  ...
  test_time <-system.time({
    ...
    for(i in 1:npc){
      ...
      f.slope <- as.formula(paste("rating ~ 1 + temp.1 + temp.2 + ascore.", additive0.sim, " + (0 +", ii, " | ID)", 
                                  sep = ""))
...
fileName <- paste("size_", smooth, "_",b.var,"_seed1_grp20-rep50.RData", sep = "") # Saving file's name
