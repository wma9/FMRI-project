...
for(r.sim in c(0.016,0.018,0.02,0.025,0.027,0.03,0.035,0.037,0.0425,0.045,0.0475,0.05,0.0576,0.065)){
  for(smooth in c(0,1)){
    for(seediter in 1:4){
      ...
      b.var <- r.sim
      fileName <- paste("bonf_homo_test_power_", smooth, "_",b.var,"_seed",
      ...
      power2.sim[[loopIndex]] <- list(Power = Power2, realTau = c(r.sim,r.sim,r.sim), smooth = smooth, single_test_sec = single_testtime)
