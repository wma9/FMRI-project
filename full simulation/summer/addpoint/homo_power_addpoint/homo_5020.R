...
clusterExport(cluster, c("nSubj", "nRep"))

for(r.sim in c(0.016,0.018,0.02,0.025,0.027,0.03,0.035,0.037,0.0425,0.045,0.0475,0.05,0.0576,0.065)){
  for(smooth in c(0,1)){
    for(seediter in 1:4){
      clusterExport(cluster, c("r.sim", "smooth","seediter")) # casting the coefficient parameter on the random effects' covariance function
      b.var <- r.sim
      fileName <- paste("homo_test_power_", smooth, "_",b.var,
                        "_seed",seediter,
                        "_grp",nSubj,"-rep",nRep,".RData", sep = "") # Saving file's name
      ...
      power2.sim[[loopIndex]] <- list(Power = Power2, realTau = c(r.sim,r.sim,r.sim), smooth = smooth)
