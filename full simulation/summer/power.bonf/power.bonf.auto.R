...
#!/usr/bin/env Rscript
args = commandArgs()
nSubj <- args[1] #20, 50, 200
nRep  <- args[2] #20, 50
seediter <- args[3] #1,2,3,4
...
for(r.sim in c(.05^2, .07^2, .1^2, .12^2, .15^2, .2^2, .23^2, .25^2, .27^2, .29^2, .31^2, .35^2, .4^2)){
  for (smooth in c(1,0)){
    clusterExport(cluster, c("r.sim", "smooth")) # casting the coefficient parameter on the random effects' covariance function
    b.var <- r.sim
    fileName <- paste("test_power_", smooth, "_",b.var,"_seed",
