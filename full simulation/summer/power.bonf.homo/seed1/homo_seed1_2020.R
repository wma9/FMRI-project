...
for(r.sim in c(.05^2, .07^2, .1^2, .12^2, .15^2, .2^2, .23^2, .25^2, .27^2, .29^2, .31^2, .35^2)){
  for(smooth in c(0,1)){
    clusterExport(cluster, c("r.sim", "smooth")) # casting the coefficient parameter on the random effects' covariance function
    b.var <- r.sim
    fileName <- paste("test_homo_power_", smooth, "_",b.var,"_seed1_grp20-rep20.RData", sep = "") # Saving file's name
